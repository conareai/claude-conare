#!/usr/bin/env node
// claude-conare SessionStart hook.
//
// Injects the precomputed Conare project brief as additionalContext before the
// model's first token. Fails silent BY DESIGN: any missing prerequisite,
...
// Wall budget for the network fetch. The hook's registered timeout is 5s;
// this keeps us well under it so the harness never has to kill us.
...
async function fetchBrief(apiKey, repo) {
  ...
  try {
    const res = await fetch(
      `${API_URL}/api/hook/brief?repo=${repo}&client=claude`,
      {
        headers: {
          Authorization: `Bearer ${apiKey}`,
          "User-Agent": `claude-conare/${VERSION}`,
        },
